...
--컬렘 레벨 제약 조건 (컬럼 선언마다 제약조건 지정)
 CREATE TABLE dept2 (
    dept_no NUMBER(2) CONSTRAINT dept2_deptno_pk PRIMARY KEY, -- 제약조건 식별자는 생략이 가능합니다. (오라클이 알아서 이름 지음)--dept2_deptno_pk는(제약조건식별자)이름이라 원하면생략가능(대신이름없음)
    dept_name VARCHAR2(14) NOT NULL CONSTRAINT dept2_deptname_uk UNIQUE,-- NOT NULL, CONSTRAINT ,UNIQUE 이거 순서는내맘임
    loca NUMBER(4) CONSTRAINT dept2_loca_locid_fk REFERENCES locations(location_id), --FOREIGN LEY
...
--테이블 레벨 제약 조건(모든 열 선언후 제약조건을 취하는 방식)
CREATE TABLE dept2 (
   dept_no NUMBER(2) , 
   dept_name VARCHAR2(14) CONSTRAINT dept2_deptname_notnull NOT NULL, 
   loca NUMBER(4)  , 
