...
--이름이 'Nancy' 인 사원보다 급여가 높은사람을 구하기
...
--밑에문장은 서브쿼리가 리턴하는 행이 여러 개라서 단일 행 연산자(> >= < <= = <>)를 사용할수없습니다. 
--단일 행 연산자 : 주로 비교 연산자 ( = < > >= <= <> )를 사용하는 경우 하나의 행만 반환해야합니다.
--이런경우에는 다중 행 연산자를 사용해야합니다
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
...
-- first_name 이 David인 사람들의 급여와 같은 급여를 받는 사람들을 조회.
SELECT * 
FROM employees
WHERE salary IN (SELECT salary FROM employees WHERE first_name = 'David');


--ANY , SOME(오라클) : 값을 서브쿼리에 의해 리턴된 각각의 값과 비교해서
--하나라도 만족하면 조회 대상에 포함됩니다
--David 라는 사람이 여러명인데 그 중에 가장 적은 급여를 받는 David 보다 급여가 높은 사람을 조회
SELECT * FROM employees
...
--David 라는 사람이 여러명인데 그중에 가장 많은 급여를 받는 David 보다 급여가 높은사람을조회
                 SELECT * FROM employees
WHERE salary > ALL (SELECT salary FROM employees
                    WHERE first_name = 'David');  
                    
                    
--EXISTS : 서브 쿼리가 하나 이상의 행을 반환하면 참으로 간주
--job_history 에 존재하는 직원이 employees에도 존재한다면 조회대상에 포함.
--서브쿼리 내에서 jh에 잇는 id와 e에잇는 id가 일치할 때마다 1이라는 값을 조회.
-- EXISTS 연산자가 1이 조회가 될때 데이터가 존재한다는 것을 판단하여 employees에서 해당사원의  모든정보를 조회.
