...
--FULL OUTER JOIN 좌측 테이블과 우측 테이블 데이터를 모두읽어 표현하는 외부조인
SELECT *
FROM INFO i FULL JOIN auth a
ON i.auth_id = a.auth_id;


-- CROSS JOIN 은 JOIN 조건을 설정하지 않기떄문에
--단순히 모든컬럼에대한 JOIN을 진행합니다
--실제로는 거의 사용하지않습니다.
SELECT * FROM info
CROSS JOIN auth;

SELECT * FROM info,auth; --이건 크로스조인 오라클문법

--여러 개 테이블 조인 -> 키 값을 찾아 구문을 연결해서 쓰면됩니다
SELECT *
FROM employees e
