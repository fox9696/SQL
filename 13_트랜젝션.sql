--오토 커밋 활성화 여부 확인
SHOW AUTOCOMMIT; --off
--오토 커밋 온
SET AUTOCOMMIT ON; --IMMEDIATE 상시커밋 *비추
--오토 커밋 오프
SET AUTOCOMMIT OFF; 

SELECT * FROM emps;


DELETE FROM emps WHERE employee_id = 100;

INSERT INTO emps
     (employee_id , last_name , email , hire_date , job_id) 
VALUES
     (304,'lee','lee1234@gmail.com',sysdate,'test');
    
--보류중인 모든 데이터 변경사항을 취소 (폐기)
--직전 커밋 단계로 회귀(돌아가기) 및 트랜잭션 종료
ROLLBACK; 


--세이브 포인트생성
--롤백할 포인트를 직접 이름을 붙여서 지정
--ANCI 표준 문법은 아니기 때문에 그렇게 권장하지는않음
SAVEPOINT insert_park;

INSERT INTO emps
     (employee_id , last_name , email , hire_date , job_id) 
VALUES
     (305,'park','park4321@gmail.com',sysdate,'test');
     
 ROLLBACK TO SAVEPOINT insert_park; --권장하지않음 롤백과 커밋의 의미가퇴색됨   


--보류중인 모든데이터 변경사항을 영구적으로 적용하면서 트랜잭션종료
-- 커밋후에는 어떠한 방법을 사용하더라도 되돌릴수없습니다.
COMMIT;







